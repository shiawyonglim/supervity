...
export default function AIInsightsPage() {
  ...
  const processInsights = (data: Insight[]) => {
    setInsights(data)
    ...
    const p: Pattern[] = data
      ...
      .map(i => ({
        name: i.title,
        frequency: (i.data?.frequency as string) || 'ongoing',
        confidence: i.confidence ?? 0,
        sample_size: (i.data?.sample_size as number) || 0,
        description: i.description,
      }))
    setPatterns(p)

    // Derive actions
    const a: ActionItem[] = data
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true)
    try {
      const result = await apiClient.post<any>('/api/insights/generate')
      if (result.insights) {
        processInsights(result.insights)
      } else {
        await fetchInsights()
      }
    } catch (err) {
